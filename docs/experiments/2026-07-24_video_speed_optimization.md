...
- 이번 작업으로 추가된 CLI 플래그(`--diffusion-step`/`--max-frames`/
  `--force-interframe-reuse`/`--no-clip`/`--recon-caption-mode`/`--packet-cache-dir`/
  `--profile`/`--profile-out`, `run_etri_video_eval.py`의 `--parallel`/`--devices`/
  `--gpu-log-interval`)의 현재 정의와 사용법은
  [protocols/evaluation.md](../protocols/evaluation.md#성능속도-옵션-opt-in)를
  기준으로 본다(이 문서에서 중복 기재하지 않음).
