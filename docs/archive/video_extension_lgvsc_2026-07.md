...
supersedes: docs/architecture/tx_rx_contract.md
---

> [← 문서 색인](../README.md)

> 이 문서는 과거 스냅샷이다. 본문에 남은 옛 문서 경로는 현재
> [문서 색인](../README.md)의 대응 문서를 사용한다.
...
> [etri_strategy.md](../current/status.md)의 "1차 구현 결과" 절이다.
>
> **2026-07 ETRI 2차 구현 반영** — §6.3 6단계의 **기초 형태**(rule-based
> verifier + 오류 유형별 controller decision, OWLv2/VQA 보강 이전)가
> `TemporalPipeline`에 옵션으로 연결됐다: `evaluators/packet_verifier.py`
> (기존 `semantic_packet_matcher` 위 wrapper + severity score),
> `controllers/verifier_controller.py` (accept / suppress_extra /
> strengthen_missing / strengthen_structure_guidance / fallback_recompute /
> keyframe_fallback), `pipelines/packet_verification.py` (연결부),
> `scripts/evaluate_video.py`의 `use_packet_verifier` + `verifier.enabled`
> 게이트(기본 OFF). **의도적으로 generate 분기(4~5단계)보다 먼저 붙였다** —
> generate가 켜지기 전에 "복원이 전송 packet과 맞는지" 판정·로그 경로부터
> 안정화해 두려는 것이다(§6.3 서두 "6단계를 우선한다" 설명과 동일 이유).
> 이 2차 구현은 candidate action(negative prompt / prompt emphasis)을
> **로그만** 남기고 샘플러에 실제로 주입하지 않는다. OWLv2/VQA 보강은
> 계획대로 5차 후속이며, generate 결합(3차)은 아래 3차 반영 문단대로 이후
> 실제로 붙었다. 진행 상태의 단일 기준은
> [etri_strategy.md](../current/status.md)의 "2차 구현 상태" 절이다.
>
> **2026-07 ETRI 3차 구현 반영** — §6.3 4단계(start-only generate 연결)의
> **기초 형태**가 붙었다: 신규 `video/video_generator.py`가
> `reuse`/`recompute`/`generate` 3-way 분기의 `generate` 쪽을 담당하고,
> `TemporalPipeline`은 reuse 이중 게이트를 통과 못 한 inter-frame 중 semantic
> delta/motion이 "너무 크지 않은" 구간만 generate 후보로 넘긴다. 3차의
> generator backend(`copy`/`interpolation`)는 **모두 mock**이며 실제 학습형
> 생성 모델이 아니다 — 이는 의도된 범위 제한이다(3차 목표는 생성 품질이
> 아니라 3-way 분기의 구조적 동작 확인). Rx-legal 경계도 명시적으로 분리했다:
> `reference_target_frame`(원본 target 프레임)은 `allow_ground_truth_reference`를
> 켠 테스트/mock 경로에서만 쓰이고, 그 사실이 결과 메타데이터에 남는다.
> bidirectional(start+end keyframe) conditioning은 `NotImplementedError`로
> 막아 4차(§6.3 5단계) 확장점으로만 남겼다. 진행 상태의 단일 기준은
> [etri_strategy.md](../current/status.md)의 "3차 구현 결과" 절이다.
>
> **2026-07 ETRI 4차 구현 반영** — §6.3 5단계(start+end bidirectional
> generation)의 **기초 형태**가 붙었다: 신규 `BidirectionalInterpolationGenerator`가
> start keyframe recon과 (다음 GOP의) end keyframe recon을 target 프레임의
> 상대 위치(`relative_position`)로 선형 보간한다. 두 keyframe recon 모두
> 수신단이 실제로 복원한 결과이므로 Rx-legal이며(3차의 ground-truth-reference
> 테스트 경로와는 다른 성격 — `mock=True` 표시는 "학습형 생성이 아님"만
> 뜻한다), `TemporalPipeline`은 bidirectional 모드에서만 keyframe recon을
> 미리 계산해 두는 prepass를 추가해 다음 GOP의 keyframe recon을 앞선 GOP의
> inter-frame 생성에 전달한다(start-only 모드는 이 prepass를 타지 않아 3차
> 단일 패스가 그대로 유지됨). end keyframe이 없는 마지막 GOP나
> `target_index`가 범위를 벗어난 경우는 `bidirectional_missing_end_policy`
> (`error` 기본 / `fallback_start_only`)로 제어한다. 같은 입력을 start-only와
> bidirectional 양쪽으로 돌려 `PTC`/`SFR`/`SDI`/`n_generate`/`n_reused`/
...
> 진행 상태의 단일
> 기준은 [etri_strategy.md](../current/status.md)의 "4차 구현 결과" 절이다.
>
> **2026-07 ETRI 5차 구현 반영** — §6.3 6단계(Rx-legal verifier)의 OWLv2/VQA
> 보강 **인터페이스**와 §6.1-c의 loop-internal/held-out 분리 **구조**가
> 코드로 붙었다: 신규 `evaluators/presence_backends.py`(clip/owlv2/vqa/gt/mock
> 공통 인터페이스 — CLIP 판정은 재구현 없이 기존 로직을 감싸기만 함),
> `evaluators/presence_calibration.py`(6가지 조합 모드 ensemble calibrator),
> `pipelines/heldout_remeasurement.py` + `scripts/remeasure_video_metrics.py`
> (1~4차 결과를 다시 읽어 clip_only vs calibrated로 재측정하고
> `metric_delta`를 남김), `evaluators/temporal_srs_calibration.py`(가중치
...
> 상태의 단일 기준은 [etri_strategy.md](../current/status.md)의 "5차 구현 결과" 절이다.
...
- **generate 전에** GOP 단위 `SegmentRecord`/`GOPSegment` 자료구조 정의
  ...
  **구현됨** (`video/segment.py::SegmentRecord`; `generation` 필드는 1차에서는
  예약(항상 None)이었으나 3차에서 실제로 채워짐 — 아래 4단계 참조. side_info는
  delta/motion 요약으로 시작)
...
> **구현 상태**: `video_generator` 인터페이스와 3-way 분기가 붙었다 — 신규
> `video/video_generator.py`(`VideoGenerator`/`CopyGenerator`/`InterpolationGenerator`
> mock backend + 실제 backend용 레지스트리), `TemporalPipeline`의
> reuse 이중 게이트를 통과 못 한 inter-frame 중 semantic delta/motion이 "너무 크지
> 않은" 구간을 `generate`로 라우팅, `use_video_gen` + `video_generator.enabled`
> 게이트(기본 OFF, `use_phase4`에 종속). **아직 아닌 것**: 실제 학습형 생성 모델
> 통합(mock만 구현), segment 단위 batched 호출(현재는 frame-wise 호출 후
> `SegmentRecord.generation`에 집계), mp4 조립 자체는 이미 1차에서 구현된
> `video_io`를 그대로 재사용(3차가 새로 만들지 않음). 자세한 내용은
> `docs/etri_strategy.md`의 "3차 구현 결과" 절 참조.
...
> **구현 상태**: mock 기준 bidirectional 조건화와 start-only 대비 비교
> 파이프라인이 붙었다 — 신규 `BidirectionalInterpolationGenerator`(선형 보간,
> `relative_position` 기반), `TemporalPipeline`의 bidirectional 전용 prepass
> (다음 GOP의 keyframe recon을 미리 계산해 "end" 조건으로 전달),
> `bidirectional_missing_end_policy`(마지막 GOP/범위 밖 처리),
> `pipelines/generation_mode_comparison.py`(같은 입력으로 두 모드를 돌려
> `PTC`/`SFR`/`SDI`/decision count를 diff). **이 mock 구현은 아래 "선행: 0단계
> spike" 요구사항과 무관하다** — 0단계는 실제 SVD/Open-Sora 같은 학습형
> 백엔드의 실현가능성을 확인하는 게이트이고, 4차는 그 학습형 백엔드가 붙기
...
> **ETRI 2차 상태 (2026-07)**: 아래 기초 형태가 구현·연결됐다 — `evaluators/packet_verifier.py`
> (severity score) + `controllers/verifier_controller.py`(오류 유형별 decision) +
> `pipelines/packet_verification.py`(`TemporalPipeline` 결과 연결, 기본 OFF 게이트).
> **아직 아닌 것 (2차 시점)**: OWLv2/VQA 보강, held-out 지표 기반 채택 판정, candidate action의
> 실제 sampler 주입 — 이들은 계획대로 5차 및 후속 항목이다.
>
> **ETRI 5차 상태 (2026-07) 추가**: OWLv2/VQA 보강의 **인터페이스**와 held-out
> 지표 분리의 **구조**가 붙었다 — `evaluators/presence_backends.py`(공통
> presence 인터페이스) + `evaluators/presence_calibration.py`(ensemble
...
- [etri_strategy.md](../current/status.md) — 핵심 한계 3가지와 현재 상태 (이 문서는 한계 3의 장기 확장 축)
- [roadmap.md](../current/roadmap.md) — 2~4단계(수신단 개선/송신단 지능화/전체 검증) 향후 계획
- [phase4.md](./phase4_2026-07.md) — 키프레임/시간적 파이프라인 (4-B)
- [framework_file_roles.md](../reference/framework_file_roles.md) — 파일별 실행 흐름 지도
