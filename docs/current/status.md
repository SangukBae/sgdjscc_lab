...
| 실제 binary packet 전송 (`transmission/`, 4/6/8/16/32-bit 양자화) | **전송 안정성·10dB fixed 양자화 operating point 확정** — 10영상×6설정 60/60 pair, 실패·NaN/Inf 0건. `fixed_float32` 대비 int16/int8/int6/int4가 모두 품질 허용 기준을 통과했고 최소 bit-depth `fixed_int4`를 선택(전송 byte -28.45%, PSNR -0.0526dB, SSIM -0.00201, LPIPS rise -0.000487). AWGN은 참고행이며 digital Pareto baseline에서 제외 — [10dB 재평가](../experiments/2026-08-28_quantization_reevaluation_10db.md), [보존 결과](../../results/quantization_reevaluation_10db_20260828/README.md) |
...
| edge·uncertainty 전송 절감 ablation | **16-profile·3-GPU 실행 harness 준비 완료, full GPU 실행 대기** — `fixed_int4` 기준선 + edge/uncertainty 각각 q4·1/2·1/4·reuse2·omit의 독립 5종 + 결합 5종. receiver-only cache/0-map 경계, label-length byte artifact 제거, component reconciliation·품질 gate·Pareto fail-closed 검증 포함 — [준비 문서](../experiments/2026-08-28_edge_uncertainty_ablation_preparation.md) |
