...
| fixed–SKEM exact matched-rate 재평가 | **3-GPU 실행·자동 검증 준비 완료, full 실행 대기** — fixed max-GOP은 유지하고 영상별 SKEM threshold/max-segment를 보정해 실제 visual-transmitting frame 수를 정확히 일치시킨다. raw bundle byte 차이 1% gate와 padding 후 exact effective byte, 100 pair/50 rate row·10dB·failure/non-finite·worker provenance를 fail-closed로 검증. 실행기: `scripts/run_fixed_skem_matched_rate_10db.sh` |
