...
④ 출력·평가
  src/sgdjscc_lab/utils/video_io.py [신규]  mp4→frames, frames→mp4 (원본 fps)
      ※ 현재 evaluate_video.py는 ordered frame folder 입력 + CSV/JSON 출력만 —
        --video in.mp4 입력과 복원 mp4 저장은 전부 신규 구현
...
| 평가 (CLIP·DISTS·LPIPS·PSNR/SSIM·downstream) | 의미 충실도 채점 | `evaluators/` — `temporal_consistency`에 temporal_srs·srs_flicker·object_identity·temporal_hallucination **이미 있음**. **object birth/death flicker·track drift·flow-warp·DISTS·downstream은 부재** | `[확장]` 신규 지표 + LGVSC 비교용 metric profile |
...
- `utils/video_io.py` [신규]: mp4 → frames, frames → mp4(원본 fps)
- 기존 **2-way 파이프라인 결과를 영상 파일로 저장** (generate 없이)

**판정 기준** — mp4 왕복에서 **프레임 수·fps·해상도·정렬 순서가 보존**되고 재조립 영상이
정상 재생되며, 현행 2-way 복원이 영상으로 나오면 진행. (MP4는 lossy 코덱이므로 픽셀
무손실이 아니라 구조·순서 보존을 기준으로 한다.)

#### 2단계 — temporal metric 정리 + flicker 확장
...
- baseline 집합(§6.1-a)을 실제 모션 비디오에서 산출

**판정 기준** — 지표가 baseline 간(copy-reuse vs per-frame) **분별력**을 보이면 진행.

#### 3단계 — segment abstraction
