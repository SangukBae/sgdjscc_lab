...
이 문서는 `docs/etri_strategy.md` "후속 딥러닝 4단계"의 **1B (실제 생성 backend
...
| 1C (`SKIM+SFA`/`SKEM+DSA` 재현 검증) | 🟡 **재현 준비 완료(구조), 실제 검증 실행은 사용자** — config 4종 + batch driver + summary 생성기 완성, 실제 10영상×4모드 GPU 실행 결과는 아직 없음. 자세한 내용은 `docs/lgvsc_1c_reproduction_readiness.md` 참조 |
...
내용은 [docs/lgvsc_1c_reproduction_readiness.md](./lgvsc_1c_reproduction_readiness.md)
...
해야 완료로 본다(`docs/etri_strategy.md`의 "1단계는 다시 세 개의 독립 완료
게이트로 관리한다" 참조).

## 관련 문서

- [etri_strategy.md](./etri_strategy.md) — "후속 딥러닝 4단계", 1A/1B 구현 결과
- [video_extension_lgvsc.md](./video_extension_lgvsc.md) — 6.0-a/6.2/6.3, LGVSC
  재현선과 ETRI 개선선 구분
- [etri_owlv2_vqa_readiness.md](./etri_owlv2_vqa_readiness.md) — 이 문서와 같은
