# ETRI Research Progress Update — Detailed Slide Notes for External Sharing

- Source deck: `ETRI_연구진행상황공유_20260816_ETRI용_업데이트.pptx`
- Reporting date: August 2026
- Number of slides: 9
- Purpose: Expand the compact slide text into a detailed explanation of the research rationale, data flow, implementation status, validation scope, metric definitions, and presentation caveats. It contains only the research content and interpretation needed for external sharing.

In these notes, “implemented” means that the relevant capability is available and has been exercised within the stated scope. It does not automatically mean that performance superiority has been demonstrated. Actual bitstream/CBR validation, full-SNR and fading experiments, fair comparisons with other models, full-video superiority of the generation modes, and GT/VLM-based Temporal SRS weight learning are explicitly treated as follow-on work.
...
Image inference, video processing, evaluation, and stage-aware training have been integrated modularly while preserving the original SGD-JSCC inference flow. Video and generation extensions are separated from the baseline image path so that the established behavior remains available when those extensions are not used.
...
PTC, SFR, and SDI extend image SRS into the temporal domain. Real SGD-JSCC reconstructions from 10 videos were evaluated in five presence modes, producing 50 jobs in total. Each job contains 100 items, and the mode-level results were aggregated into an overall summary.
...
Repeated execution, per-video result separation, summary generation, status tracking, and reporting are automated. The slide cites 1,133 passing automated tests as evidence of structural stability and regression coverage. This does not by itself prove algorithmic superiority; it shows that the prepared capabilities and baseline-preservation conditions operate as expected.

### Lower Table — Levels of Validation

- **Structural validation — complete**: the system runs, the established behavior remains available when extensions are not used, and automated tests pass.
...
Use this slide as the boundary against overclaiming. “The system runs,” “the intended quantity is measured,” and “the method outperforms alternatives” are different claims. The first two have been established within the stated scope; only part of the third has been established.
...
AWGN is the implemented baseline channel. Rayleigh fading, fast fading, and packet loss are also included as extension targets; their presence in the diagram does not mean the final all-condition comparison is complete. The Edge JSCC path simulates its own channel behavior within that network path.
...
- Image evaluation covers PSNR, SSIM, LPIPS, CLIP, object preservation/missing/additional, hallucination, and SRS.
- Video evaluation covers PTC, SFR, SDI, temporal SRS, and related measures.
- The dashed source-to-output comparison emphasizes semantic agreement with the source, not only perceived quality.

### Interpretation Caveat

Do not imply that caption and edge information experience exactly the same corruption as the principal latent. The slide explicitly distinguishes the error-free caption-side-information assumption and the separate Edge JSCC link. The video keyframe/segment/generate extensions are also kept distinct from the baseline image flow, so results from the two conditions should not be mixed.
...
The MP4 I/O process extracts frames and writes reconstructed frames back to MP4. Keyframe selection can respond to scene transitions and optionally use PSSS. Keyframe and segment selections and decisions are recorded for reproducibility and post-hoc analysis.
...
If packet matching or VQA is used to change reconstruction and then the same metric is reported as the final result, the evaluation becomes circular and vulnerable to metric gaming. Reconstruction-time decisions and held-out final evaluation are therefore assigned separate roles, and final evaluation is rerun through an independent procedure. Slide 7 explains this principle.
...
- **GT-object-only ensemble**: a closed-world mode for object-preservation claims.
- **Open-world ensemble**: retains non-GT candidates for additional-object and hallucination analysis.
- **Unfiltered ensemble**: diagnostic only because caption-noun contamination may be present.
...
The preparation for weight management and least-squares fitting is in place. Final weights have not yet been learned from real GT annotations or an independent VLM judge. The dashed border communicates that the infrastructure exists but real-data calibration is still future work.
...
MP4 I/O, segment structures, the semantic/motion gate, and PTC/SFR/SDI are complete. Evidence includes a real-model 10-video baseline, reconstructed videos, per-frame and temporal evaluation results, and keyframe/segment records. These results show that the full flow runs from input video through reconstruction to evaluation.
...
The main contribution is not merely attaching a video front end to an image restoration model. The project preserves the baseline flow while connecting temporal transmission decisions, packet-based receiver verification, held-out semantic evaluation, and rate accounting in a reproducible experimental framework. The remaining work is to turn that framework into a matched-rate, fair-comparison demonstration of quantitative performance gains.
