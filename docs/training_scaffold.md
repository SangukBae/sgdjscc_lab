...
논문 근거:
- Stage 1: *"trained ... under a fixed noisy channel setting (AWGN, SNR=10dB)"*,
  `L = ‖x−x̂‖² + λ·L_GAN` (eq. 7). *"The JSCC model is fixed after this stage."*
- Stage 2: Algorithm 1 — `t~U(0,1)`, `β̄_t=S(t)`, `f_t=√(1−β̄_t)f0+√β̄_t·n`,
  `∇‖f0−ε(f_t,β̄_t)‖²`. MDTv2의 masked-latent modeling으로 masked 항 추가.
- Stage 3: *"the parameters of the original text-guided DM are frozen, and only
  the parameters of the DiT blocks handling the structural semantic features are
  updated."*
