...
- generated: 2026-08-27T22:42:50.196090+00:00
- output_root: outputs/f32dig_20260827_164017
- stage_failures: 0
- execution_mode: three_gpu_parallel
- physical_devices: 0,1,2

**The root-cause section consolidates each instrumented stage's own verdict (see docs/protocols/float32_digital_diagnostics.md for the classification criteria); it is NOT itself a new judgment, only a rollup of what each stage's own summary.json/verdicts.jsonl already recorded. Only `ablation == "baseline"` AND `status == "final"` rows feed any dominant-verdict tally below -- auxiliary edge-equalizing ablations and still-provisional baseline rows are listed separately, never summed into it. When stages overlap, the richest evidence wins (`baseline_with_vae_direct` > `baseline_only` > provisional); only disagreements at the same evidence level are conflicts. Stages run with `--no-instrument-tensors` have no root-cause verdict; their baseline CSV rows are instead included in a separate metric-only quality assessment using the documented 1 dB quality and 0.01 dB path-parity thresholds.**

## per-stage verdict summary (baseline, final only)

| stage | n_frames_processed | dominant_verdict | baseline verdict counts | metric-only quality | evidence levels | provisional | auxiliary | failed_cases |
|---|---:|---|---|---|---|---:|---:|---:|
| stage3_single_frame_paths | 1 | no_issue_detected | no_issue_detected=1 | (none) | baseline_only=1 | 0 | 0 | 0 |
| stage4_single_frame_ablations | 1 | no_issue_detected | no_issue_detected=1 | (none) | baseline_with_vae_direct=1 | 0 | 2 | 0 |
| stage5_paired_frames | 20 | no_issue_detected | no_issue_detected=20 | (none) | baseline_with_vae_direct=20 | 0 | 0 | 0 |
| stage6_core_conditions/worker_00_normal_motion | 100 | (none) | (none) | no_issue_detected=100 | (none) | 0 | 0 | 0 |
| stage6_core_conditions/worker_01_semantic_change | 100 | (none) | (none) | no_issue_detected=100 | (none) | 0 | 0 | 0 |
| stage6_core_conditions/worker_02_scene_cut | 100 | (none) | (none) | no_issue_detected=100 | (none) | 0 | 0 | 0 |

## overall (baseline, final only, deduplicated across stages by (video, frame))

- dominant_verdict: `no_issue_detected`
  - `no_issue_detected`: 20

## metric-only baseline quality assessment (no tensor/root-cause verdict)

- thresholds: AWGN − digital wire PSNR < 1.0 dB; |digital in-process − wire PSNR| <= 0.01 dB; wire round-trip bit-exact
- `no_issue_detected`: 300
- digital wire − AWGN PSNR: mean=0.721163 dB, min=-0.316044 dB, max=2.140766 dB
- max |digital in-process − wire PSNR|: 0.000751495 dB
- bit-exact wire rows: 300/300

## auxiliary evidence (serialized_raw_edge / awgn_edge_retransmit, never summed into the overall count)

- `transport_delta_detected`: 2
